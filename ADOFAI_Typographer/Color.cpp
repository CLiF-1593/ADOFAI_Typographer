...
Color::RGB::RGB(int r, int g, int b, int a) {
	this->r = r;
	this->g = g;
	this->b = b;
}
...
RGB Color::WindowsSTD::theme_color(0, 0, 0);

void Color::WindowsSTD::Init() {
	UISettings const ui_settings{};      
	auto const accent_color{ ui_settings.GetColorValue(UIColorType::Accent) };
	auto hsv = RGB(accent_color.R, accent_color.G, accent_color.B).to_HSV();
	hsv.s = 50;
	hsv.v = 85;
	WindowsSTD::theme_color = hsv.to_RGB();
}

RGB Color::WindowsSTD::GetThemeColor() {
	return WindowsSTD::theme_color;
}
