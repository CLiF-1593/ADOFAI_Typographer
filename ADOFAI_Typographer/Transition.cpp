...
double Transition::Calculate(double progress, TransitionType type) {
	...
	case IN_CUBIC:
		...
		break;
	case OUT_CUBIC:
		result = 1 - pow(1 - progress, 3);
		break;
	case IN_OUT_CUBIC:
		result = progress < 0.5 ? 4 * progress * progress * progress : 1 - pow(-2 * progress + 2, 3) / 2;
		break;
	case IN_QUART:
		result = progress * progress * progress * progress;
		break;
	case OUT_QUART:
		result = 1 - pow(1 - progress, 4);
		break;
	case IN_OUT_QUART:
		result = progress < 0.5 ? 8 * progress * progress * progress * progress : 1 - pow(-2 * progress + 2, 4) / 2;
		break;
	case IN_QUINT:
		result = progress * progress * progress * progress * progress;
		break;
	case OUT_QUINT:
